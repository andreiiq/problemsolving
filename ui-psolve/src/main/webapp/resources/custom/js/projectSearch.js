	        ...
	        $.ajax({
	            ...
	            success: function (data) {
	                ...
	                for (var key in data.tasks) {
	                	  var html = '<div class="row" style="margin-top: 0.5em"><form class="form-horizontal" role="form" method="GET" action="'+ ctx +'/getTask"> <div type="button" class="project-button text-justify well img-rounded "> <h6 class="content-subtitle"><input type="hidden" name="taskId" value="'+ data.tasks[key].id +'"><span class="hover-color"><strong>' + data.tasks[key].title + '</strong></span> </h6> <div class="projectDescription"><b>Course: </b>'+data.course +' <br> <b>Points: </b>'+ data.tasks[key].points +'<br>' + data.tasks[key].description + '</div></div></form></div';
...
//	                buildPageNavigation(data.numberOfPages, data.currentPage);
    ...
    $('#filter-search-results').on('click', '#next-page', function () {
    	var currentPage = $('#current-page-number-ftasks');
    	currentPage.val(parseInt(currentPage.val()) + 1);
        $('#filter-btn-submit').trigger( "click" );
    });
    
    $('#filter-search-results').on('click', '#previous-page', function () {
    	var currentPage = $('#current-page-number-ftasks');
    	currentPage.val(parseInt(currentPage.val()) - 1);
        $('#filter-btn-submit').trigger( "click" );
    });
    function appendSearchResults(data) {
  	  var html = '<div class="row" style="margin-top: 0.5em"><form class="form-horizontal" role="form" method="GET" action="'+ ctx +'/getTask"> <div type="button" class="project-button text-justify well img-rounded "> <h6 class="content-subtitle"><input type="hidden" name="taskId" value="'+ data.id +'"><span class="hover-color"><strong>' + data.title + '</strong></span> </h6> <div class="projectDescription"><b>Course: </b>'+data.course +' <br> <b>Points: </b>'+ data.points +'<br>' + data.description + '</div></div></form></div';
    ...
    function buildPageNavigation(numberOfPages, currentPage) {
    	...
    	if(currentPage != 0) {
    		html = html.concat('<li> <a id="previous-page" href="#" aria-label="Previous"> <span aria-hidden="true">&laquo;</span> </a> </li>');
    	} 
        ...
        html = html.concat('<li> <a id="next-page" href="#" aria-label="Next"> <span aria-hidden="true">&raquo;</span> </a> </li>');
        }
    ...
    function buildSelectedProjectBasic(data) {
    	...
    	var htmlTaskInfo='<div class="form-group row">'+
    		...
    		'<div class="col-sm-10">'+ data.title +'</div>'+ htmlAssignationButton +
        ...
        for (var key in data.subtasks) {
    		...
    		'<div class="panel panel-default">'+
    			'<div class="panel-heading">'+
    				'<h4 class="panel-name">'+
    					'<a data-toggle="collapse" href="#collapse-subtask-selected-project-'+ key +'"> Subtask: '+ data.subtasks[key].title +'</a>'+
    				'</h4>'+
    			'</div>'+
        ...
        htmlTaskInfo = htmlTaskInfo + htmlSubtaskInfo ;
        return htmlTaskInfo;
    }
				...
				'<div class="panel-body">'+
					'<form enctype="multipart/form-data" class="form-register"'+
						...
						'<input class="upload-solution" type="file" class="file"'+
							'name="file" />'+
						'<input type="hidden" value="'+data.id+'" name="taskId" />'+
					'</form>'+
				'</div>'+
			'</div>'+
		'</div>'+
    			...
    			'<div class="modal-content">'+
    				...
    				'<div class="modal-body">'+
    					'<div class="form-group row">'+
    						...
    						' <input type="text" class="selected-pr-task-grade form-control" name="grade">'+
    					'</div>';
    					...
    					'<div class="form-group row">'+
    						...
    						'<input type="text" class="form-control" name="subtasks['+ key+'].grade">'+
    					'</div>';
    					}
